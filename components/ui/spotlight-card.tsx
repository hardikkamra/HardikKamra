...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  useEffect(() => {
    ...
    const removePointer = () => {
      card.style.setProperty('--x', '-1000');
      card.style.setProperty('--y', '-1000');
    };

    card.addEventListener('pointermove', syncPointer);
    card.addEventListener('pointerleave', removePointer);
    
    // Initialize glow offscreen
    removePointer();

    return () => {
      card.removeEventListener('pointermove', syncPointer);
      card.removeEventListener('pointerleave', removePointer);
    };
  }, []);
